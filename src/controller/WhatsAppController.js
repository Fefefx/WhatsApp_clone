...
export class WhatsAppController {
    ...
    checkNotifications() {
        //Verifica se o browser suporta o recurso de notificações
        if (typeof Notification === 'function') {
            ...
            this.el.alertNotificationPermission.on('click', e => {
                Notification.requestPermission(permission => {
                    //Se o usuário permitiu notificações
    ...
    notification(data) {
        if (Notification.permission === 'granted' && !this._active) {
            ...
            sound.currentTime = 0;
            sound.play();
            //Mantém a notificação por três segundos na tela e após isso a fecha.
            setTimeout(() => {
                if (n)
                    n.close();
            }, 3000);
    ...
    setActiveChat(contact) {
        ...
        Message.getRef(this._contactActive.chatId).orderBy('timeStamp').onSnapshot(docs => {
            //Identifica a posição do scroll no painel
            let scrollTop = this.el.panelMessagesContainer.scrollTop;
            /*Calcula a diferença entre o tamanho do container e o tamanho do conteúdo
            dentro dele acessível descendo o scroll.*/
            ...
            docs.forEach(doc => {
                ...
                //Verifica se a mensagem não é minha e se já foi enviada
    ...
    elementsPrototype() {
        ...
        //Foi usado HTMLFormElement porque esse tipo é tratado pelo FormData
    ...
    initEvents() {
        ...
        this.el.inputProfilePhoto.on('change', e => {
            if (this.el.inputProfilePhoto.files.length > 0) {
                ...
                Upload.send(file, this._user.email).then(urlFile => {
                    this._user.photo = urlFile;
                    this._user.save().then(() => {
        ...
        this.el.btnAttach.on('click', e => {
            //O clique não dispará eventos configurados nos elementos ancestrais
            e.stopPropagation();
            this.el.menuAttach.addClass('open');
            //O bind é necessário para que o escopo não mude para o document
        ...
        this.el.btnSendPicture.on('click', e => {
            ...
            */
            ...
            picture.onload = e => {
                ...
                context.scale(-1, 1);
                //Desenha a imagem dentro do canvas espelhado
        ...
        this.el.inputDocument.on('change', e => {
            if (this.el.inputDocument.files.length) {
                ...
                this._documentPreviewController.getPreviewData().then(result => {
                    ...
                    switch (file.type) {
                        case 'application/vmd.ms-excel':
                        case 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':
                            this.el.iconPanelDocumentPreview.className = 'jcxhw icon-doc-xls';
                            break;
                        case 'application/vmd.ms-powerpoint':
        ...
        this.el.inputText.on('keypress', e => {
            if (e.key === 'Enter' && !e.ctrlKey) {
                //É possível cancelar o comportamento porque a tecla está sendo pressionada
                e.preventDefault();
                this.el.btnSend.click();
            }
        });
        this.el.inputText.on('keyup', e => {
            //Verifica se existem caracteres no inputText
        ...
        this.el.panelEmojis.querySelectorAll('.emojik').forEach(emoji => {
            emoji.on('click', e => {
                ...
                //Clono o elemento para inserí-lo no campo de mensagem várias vezes
                ...
                //Recupera o ponto do cursor do teclado
                let cursor = window.getSelection();
                if (!cursor.focusNode || !cursor.focusNode.id == 'Input-Text') {
                    this.el.inputText.focus();
                    cursor = window.getSelection();
                }
                //Interage com um intervalo de caracteres
                let range = document.createRange();
                //Localiza o início do que foi selecionado
                range = cursor.getRangeAt(0);
                //Apaga o conteúdo do intervalo
                range.deleteContents();
                //Trata fragmentos da string
                let frag = document.createDocumentFragment();
                frag.appendChild(img);
                range.insertNode(frag);
                range.setStartAfter(img);
                //Dispara o evento de keyup
