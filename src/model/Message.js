    ...
    getViewElement(me = true) {
        ...
        switch (this.type) {
            ...
            case 'document':
                    ...
                    <div class="_3_7SH _1ZPgd">
                        <div class="_1fnMt _2CORf">
                            <a class="_1vKRe" href="#">
                                ...
                                <div class="_12xX7">
                                    <div class="_3eW69">
                                        <div class="JdzFp message-file-icon icon-doc-${this.getIconFile(this.fileType)}"></div>
                                    </div>
                                    <div class="nxILt">
                            ...
                            <div class="_3cMIj">
                                ...
                                <span class="message-file-type">${Format.mimeTypeToExtension(this.fileType)}</span>
                                <span class="PyPig message-file-size">${Format.byteToLargerUnit(this.size)}</span>
                            </div>
                            <div class="_3Lj_s">
                                <div class="_1DZAH" role="button">
                                    <span class="message-time">${Format.timeStampToTime(this.timeStamp)}</span>
                ...
                `;
                if (this.info) {
                    div.querySelector('.message-file-info').innerHTML = this.info;
                    div.querySelector('.message-file-type').classList.add('Pypig');
                }
                div.on('click', e => {
            ...
            case 'audio':
                ...
                //Callback disparada quando os dados foram carregados
                audioEl.onloadeddata = e => {
                    loadEl.hide();
                    btnPlay.show();
                }
                //Responde a reprodução do áudio 
                audioEl.onplay = e => {
                    btnPlay.hide();
                    btnPause.show();
                }
                //Responde a pausa do áudio
                audioEl.onpause = e => {
                    audioDuration.innerHTML = Format.toTime(this.duration * 1000);
                    btnPlay.show();
                    btnPause.hide();
                }
                //Responde ao término da reprodução do áudio
                audioEl.onended = e => {
                    audioEl.currentTime = 0;
                }
                //Responde quando o tempo do áudio é modificado
                audioEl.ontimeupdate = e => {
                    ...
                    //Verifica se a reprodução foi pausada
                    if (audioEl.paused) {
                        btnPlay.show();
                    } else {
                        btnPause.show();
            ...
            Message.upload(file, from).then(downloadURL => {
                ...
                if (filePreview) {
                    Message.upload(filePreview, from).then(urlPreview => {
                        let downloadPreview = urlPreview;
                        msgRef.set({
                            content: downloadFile,
    ...
    static send(chatId, from, type, content) {
        return new Promise((s, f) => {
            Message.getRef(chatId).add({
                ...
                //Merge manda mesclar as informação do Cloud Storm com o novo status
                docRef.set({
                    status: 'sent'
                }, {
                    merge: true
